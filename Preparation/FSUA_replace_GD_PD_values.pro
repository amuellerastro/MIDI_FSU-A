...
;paths
  ...
  ;if we use fringe scans for calibtrating the FSU use FringeScan directory
...
;for xx=19,n_elements(fsu)-1 do begin
...
;   print, 'Record '+strcompress(xx+1, /rem)+' / '+strcompress(n_elements(fsu), /rem)
  ...
  if (calquest eq '2') then caldir = pathlabcal+'LabCal_'+labcal[xx]+'/'
...
;read in data array as structure for dark and flats
  st_d1 = mrdfits(darkfile,'IMAGING_DATA_FSUA', /silent)
  st_f1 = mrdfits(flatfile1,'IMAGING_DATA_FSUA', /silent)
  st_f2 = mrdfits(flatfile2,'IMAGING_DATA_FSUA', /silent)
...
;readin raw data, i.e. measured fluxes
  st = mrdfits(obsfile,'IMAGING_DATA_FSUA', /silent)
...
;write modified FSUA file
  ...
  for j=0,11 do begin
    ...
    if (strmatch(obsfile,'*/FSUA_OBS*') eq '1') then begin
    ...
    if (strmatch(obsfile,'*/PACMAN_GEN*') eq '1') then begin
  ...
  st1 = mrdfits(obsfile, 'IMAGING_DATA_FSUA', /silent)	;original file
  st2 = mrdfits(pathdata+fsu[xx],'IMAGING_DATA_FSUA', /silent)
;   st1 = mrdfits(obsfile, 6, /silent)	;original file
;   st2 = mrdfits(pathdata+fsu[xx],6, /silent)
  window, 0, xs=1500, ys=800
    !p.multi=[0,1,2]

    plot, st1.gd, xst=1, yst=1, yr=[-12d-6,12d-6]
