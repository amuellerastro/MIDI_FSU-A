...
pro FSU_FringeTrackPerformance
...
;pathunwrap = '/media/disk/MIDI_FSUA/COMM'+obsnum+'/MIDIreduced_Koresko_StandardMask/'
pathunwrap = '/media/disk_MIDIFSU/MIDI_FSUA/MIDI_FSUA_'+obsnum+'/MIDIreduced_Koresko_CustomMask/'
...
openw, lun, path+'FSUA_FringeTrackPerformance_'+obsnum+'.txt', width=1400,/get_lun;, /append
; printf, lun, '## The RMS value of the phasedelay is the STDDEV of written PD values, i.e. computed on a wrapped phase!'
; printf, lun, '                             PACMAN file                Star   Freq. [Hz]   LR St.7   St.5-7 [%]     rmsGD_7   rmsGD_5-7     rmsPD_7   rmsPD_5-7 [nanometer]'
printf, lun, '                             PACMAN file                Star FSU_Freq. [Hz]   LR St.7   St.5-7 [%]     rmsGD       rmsPD [nm] IRIS DIT [ms]'
...
;   ;unwrap_disp, id[i], miditime[i], path, time, pd, gd, state, wl_center, /sav
; ;
; 
;   if (obsnum eq 'P88' and id[i] ne 'HD100546') then begin
;   ;if (obsnum eq 'P88' and i ne 37) then begin
;     unwrap_disp, id[i], miditime[i], path, time, (pd-(gd*(2.d0*!DPI)/wl_center)), -1.d0*gd, state, wl_center, /sav
;   endif else begin
;     unwrap_disp, id[i], miditime[i], path, time, (pd-(gd*(2.d0*!DPI)/wl_center)), -1.d0*gd, state, wl_center, /sav, box=10
;   endelse
; ; d=p-g
; ; p=d+g	/ p=d, d=p, g=-g


  restore, pathunwrap+id[i]+'_'+miditime[i]+'_unwrap.sav'
  ;spawn, 'rm '+path+id[i]+'_'+miditime[i]+'_unwrap.sav'
  ;in this case disp is now pdel
  ;pdel = disp

  ;plot, pdel

;   idx = where(finite(pdel) eq 1)
;   if (idx[0] ne -1) then begin 
;     pdel = pdel[idx]
;     gdel = gdel[idx]
;   endif else begin
...
;   endelse
  ...
  rmsGD = stddev(gdel, /nan, /double)*1.d9
  rmsPD = stddev(pdel, /nan, /double)*1.d9
...
;   printf, lun, pf[i], id[i], freq, lr7*100., lr57*100., rmsGD7, rmsGD57, rmsPD7, rmsPD57, format='(a40,a20,i10,2f13.2,4f12.2)'
  printf, lun, pf[i], id[i], freq, lr7*100., lr57*100., rmsGD, rmsPD, irisdit, format='(a40,a20,i10,2f13.2,2f12.2,i13)'
