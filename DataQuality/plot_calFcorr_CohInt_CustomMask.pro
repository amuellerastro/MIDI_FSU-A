pro plot_calFcorr_CohInt_CustomMask
...
resultpath = mainpath+'Plots_CalFcorr_CohInt_CustomMask/'
file_mkdir, resultpath

;data path
path1 = mainpath+'MIDIreduced_CohInt_CustomMask/'
...
;for i=3,3 do begin
  ...
  set_plot, 'ps'
  device, isolatin=1
  device, filename=resultpath+id[i]+'_'+time[i]+'_CalFcorr_CohInt_CustomMask.ps', /color,XSIZE=17, YSIZE=10, XOffset=xoffset, YOffset=yoffset

;read in data in chronological order - extract time stamps
  ;wavelength
  readcol, path1+id[i]+'_'+time[i]+'.calcorr', wl, fcorr1, fcorrerr1, format='d,d,d', /silent
...
;   fcorrerr3 = fcorrerr3[idx]
  ...
  ;yr = mima([(fcorr2+fcorrerr2)+(fcorr2+fcorrerr2)*0.1])
   yr = mima([(fcorr1+fcorrerr1)+(fcorr1+fcorrerr1)*0.1])
  yr[0] = 0.
...
;   oploterror, wl, fcorr3, fcorrerr3, color=fsc_color('red'), errcolor=fsc_color('red'), /nohat
  ...
  spawn, 'gv '+resultpath+id[i]+'_'+time[i]+'_CalFcorr_CohInt_CustomMask.ps'
