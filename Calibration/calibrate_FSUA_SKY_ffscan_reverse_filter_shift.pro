...
function getFSUA_Dark, pathdf, darkfile
  ...
  if (strmatch(darkfile, 'PACMAN*') eq 1) then st_d1 = mrdfits(pathdf+darkfile,7, /silent) else $
...
; 
; 	if (i eq 0) then dark[i,j] = median(st_d1.data1[j])
; 	if (i eq 1) then dark[i,j] = median(st_d1.data2[j])
; 	if (i eq 2) then dark[i,j] = median(st_d1.data3[j])
; 	if (i eq 3) then dark[i,j] = median(st_d1.data4[j])
; 
;       endif else begin
	...
	if (i eq 0) then dark[i,j] = median(st_d1.data1[j]-st_d1.data5[0])
	if (i eq 1) then dark[i,j] = median(st_d1.data2[j]-st_d1.data5[1])
	if (i eq 2) then dark[i,j] = median(st_d1.data3[j]-st_d1.data5[2])
	if (i eq 3) then dark[i,j] = median(st_d1.data4[j]-st_d1.data5[3])
...
; ; 	dark[i,j] = median(st_d1.data1[j]-st_d1.data5[0])
; ; 	flat1[i,j] = median(st_f1.data1[j]-st_f1.data5[0])-dark[i,j]
; ; 	flat2[i,j] = median(st_f2.data1[j]-st_f2.data5[0])-dark[i,j]
; ;       endif
; ; 
; ;       if (i eq 1) then begin 
; ; 	dark[i,j] = median(st_d1.data2[j]-st_d1.data5[1])
; ; 	flat1[i,j] = median(st_f1.data2[j]-st_f1.data5[1])-dark[i,j]
; ; 	flat2[i,j] = median(st_f2.data2[j]-st_f2.data5[1])-dark[i,j]
; ;       endif
; ; 
; ;       if (i eq 2) then begin 
; ; 	dark[i,j] = median(st_d1.data3[j]-st_d1.data5[2])
; ; 	flat1[i,j] = median(st_f1.data3[j]-st_f1.data5[2])-dark[i,j]
; ; 	flat2[i,j] = median(st_f2.data3[j]-st_f2.data5[2])-dark[i,j]
; ;       endif
; ; 
; ;       if (i eq 3) then begin
; ; 	dark[i,j] = median(st_d1.data4[j]-st_d1.data5[3])
; ; 	flat1[i,j] = median(st_f1.data4[j]-st_f1.data5[3])-dark[i,j]
; ; 	flat2[i,j] = median(st_f2.data4[j]-st_f2.data5[3])-dark[i,j]
; ;       endif
; ; 
; ;     endfor
; ; 
...
pro calibrate_FSUA_SKY_ffscan_reverse_filter_shift, dodark=dodark, nodark=nodark
...
;define path and fsu 2nd fringe scan file
  path = ''
  print, ''
  read, 'Enter path, e.g. /media/amueller/disk_MIDIFSU/MIDI_FSUA/MIDI_FSUA_P90RATZKA/FSUAdata/FringeScan: ', path
...
;   path = '/home/amueller/work/MIDI_FSUA/Pipeline/Calibration/'
  ...
  tmp = file_search(path+'FSUA_SECOND_FRINGE_SCAN_*.fits', count=nf)
...
; idxscan = idxscan[0:50]
; print, ''
; print, 'REMOVE LINE 480'
; print, ''
...
;create single array instead having 1 array for each channel
...
for i=0,nscans-1 do begin
 ...
 ;correct for fringe position using fitted SNR position wrt to base value
  grid[i,*] = grid[i,*] + (base-filt_snr_pos[i])	;now fringes are centered w.r.t. base
  ...
  if (state[i,0] eq 21.d0) then begin
    ...
    gd[i,*] = reverse(transpose(gd[i,*]))

    for j=0,3 do begin

      for k=0,5 do begin

	filtABCD[i,j,k,*] = reverse(transpose(filtABCD[i,j,k,*]))
  ...
  proceeding_text,loop=(nscans), i=i, prompt='> Reverse scan direction         '+string(i+1,form='(I4)')

endfor


; window, 0, xs=1500, ys=500
; plot, grid[0,*], filtabcd[0,0,0,*], /nodata, xr=[-0.00122,-0.00118], xst=1
; for i=0,167 do begin
; oplot, grid[i,*], filtabcd[i,0,0,*]
; hak
; endfor
...
;interpolate filtered scans w.r.t. refgrid

regridABCD = dblarr(nscans,4,6,ndata)

for i=0,nscans-1 do begin

  if (i gt 0) then begin	;because we use grid of first scan as reference

    for j=0,3 do begin

      for k=0,5 do begin

	regridABCD[i,j,k,*] = normalize(interpol(filtABCD[i,j,k,*], grid[i,*], grid[0,*],/spline))
      ;interpolate does not produce runaway of the edges if grid no longer in grid[0,*]
      ;another option would be iinterpol (not the same as interpol) but look wired, i.e. sidelobes are more prominent
; 	regridABCD[i,j,k,*] = interpolate(filtABCD[i,j,k,*], grid[i,*], grid[0,*])
    ...
    regridABCD[0,*,*,*] = filtABCD[0,*,*,*]

  endelse

  proceeding_text,loop=(nscans), i=i, prompt='> Interpolate scans w.r.t. ref.  '+string(i+1,form='(I4)')

endfor

;grid is for all scans the same now
refgrid = transpose(grid[0,*])

;cut fringes out because of the edges which influence the fringe fit if not interpolated
tmp = refgrid-base
idx = where(tmp ge -100.d-6 and tmp le 100.d-6)
refgrid = refgrid[idx]
regridABCD = regridABCD[*, *, *, idx]
ndata = n_elements(refgrid)

;=================================================================================================

;average all scans

aveABCD = dblarr(4,6,ndata)
...
for i=0,3 do begin
  ...
  for j=0,5 do begin
    ...
    for k=0,ndata-1 do begin
      ...
      aveABCD[i,j,k] = mean(regridABCD[*,i,j,k])

    endfor

    aveABCD[i,j,*] = normalize(aveABCD[i,j,*])

  endfor

  proceeding_text,loop=(4), i=i, prompt='> Average scans                  '+string(i+1,form='(I4)')
...
fitparams = dblarr(4,6,6) & fitparamserr = fitparams	;channel-pixel-params
fringefit = dblarr(4,6,ndata)

  ;p0: Intensity, p1: wavelength, p2: bandwidth, p3: ZOPD offset, p4: Ioffset, p5: Phase
startval = [[500000.d0,  2.21d-6, 4.8d-7, base, 0.5d0, 0.d0],$	;base only dummy, use filt_snr_pos
	    [1500000.d0, 2.00d-6, 1.7d-7, base, 0.5d0, 0.d0],$
	    [1500000.d0, 2.12d-6, 1.7d-7, base, 0.5d0, 0.d0],$
	    [1500000.d0, 2.24d-6, 1.7d-7, base, 0.5d0, 0.d0],$
	    [1500000.d0, 2.36d-6, 1.7d-7, base, 0.5d0, 0.d0],$
	    [1500000.d0, 2.45d-6, 1.7d-7, base, 0.5d0, 0.d0]]
...
;do not change the order of the loop, have to compute white pixels first
for j=0,5 do begin	;FSU channels

  ;use the ZOPD offset of the white light pixels as starting value for the individual channels
  ;because they are narrow offset much better determined
  if (j gt 0) then begin

    newbase = median(fitparams[*,0,3])
    newbaseerr = stddev(fitparams[*,0,3])

    startval[3,j] = newbase
;     pi[3].limited[0] = 1
;     pi[3].limited[1] = 1
;   ;   pi[3].limits[0] = newbase-newbaseerr
;   ;   pi[3].limits[0] = newbase+newbaseerr
;     pi[3].limits[0] = newbase-2.d-6
;     pi[3].limits[1] = newbase+2.d-6

  endif

  pi[1].limited[0] = 1
  pi[1].limited[1] = 1
  pi[1].limits[0] = startval[1,j]-0.1d-6
  pi[1].limits[1] = startval[1,j]+0.1d-6

  for i=0,3 do begin	;spectral pixel

      lcoh = (startval[1,j]^2.-(startval[2,j]^2.)/4.d0)/startval[2,j]
      weight[i,j,*] = (abs((sin(!DPI*(refgrid-startval[3,j])/lcoh))/(!DPI*(refgrid-startval[3,j])/lcoh)))^2.
;       weight[i,j,*] = (abs((sin(!DPI*(refgrid-startval[3,j])/lcoh))/(!DPI*(refgrid-startval[3,j])/lcoh)))

      tmpparam = mpfitfun('fit_fringe', refgrid, aveABCD[i,j,*], dummy_err, startval[*,j], weights=weight[i,j,*], maxiter=500, niter=niter, status=status, bestnorm=bestnorm, yfit=yfit, perror=perror, dof=dof, /nan, parinfo=pi, /quiet)
      fitparams[i,j,*] = tmpparam
      fitparamserr[i,j,*] = perror
      fringefit[i,j,*] = yfit

      wlen[i,j] = fitparams[i,j,1]
      wlenerr[i,j] = perror[1]
...
;     !p.multi=[0,1,6]
;     for j=0,5 do begin
;       plot, refgrid, aveABCD[i,j,*], xst=1, xr=[base-100d-6, base+100d-6]
;       oplot, refgrid, weight[i,j,*], color=rgb(0,255,0)
;       oplot, refgrid, fringefit[i,j,*], color=rgb(255,0,0)
;     endfor
;     !p.multi=[0,1,0]
;     hak

  proceeding_text,loop=(4), i=i, prompt='> Fit fringe package             '+string(i+1,form='(I4)')

endfor

lambda = dblarr(6) & lambdaerr = lambda
for i=0,5 do begin

  lambda[i] = median(wlen[*,i])	;median waelength of spectral pixel of all channels
  lambdaerr[i] = stddev(wlen[*,i])
...
window, 1, xs=1800, ys=1000
  ...
  for j=0,5 do begin
    ...
    for i=0,3 do begin
      ...
      plot, (refgrid)*1.d6, aveABCD[i,j,*], xst=1, charsize=2, xr=[base-100.d-6,base+100.d-6]*1.d6
      oplot, (refgrid)*1.d6, fringefit[i,j,*], color=rgb(255,0,0)
...
hrgrid = dblarr(2000)
ndata = n_elements(hrgrid[*])
hrABCD = dblarr(4, 6, ndata)	;computed fringe package

hrgrid = linspace(base-100.d-6, base+100.d-6, ndata)

for i=0,3 do begin

  for j=0,5 do begin

    hrABCD[i,j,*] = normalize(fit_fringe(hrgrid, reform(fitparams[i,j,*])))

  endfor

  proceeding_text,loop=(4), i=i, prompt='> Create high resolution fringes '+string(i+1,form='(I4)')

endfor

;   window, 0, xs=1500, ys=1000	;white light pixel
;   !p.multi=[0,1,6]
;   plot, hrgrid, hrABCD[0,0,*], xst=1, xr=[base-12.d-6,base+12.d-6]
;     oplot, hrgrid, hrABCD[1,0,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,0,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,0,*], color=fsc_color('green')
...
;   plot, hrgrid, hrABCD[0,1,*], xst=1, xr=[base-30.d-6,base+30.d-6]
;     oplot, hrgrid, hrABCD[1,1,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,1,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,1,*], color=fsc_color('green')
;   plot, hrgrid, hrABCD[0,2,*], xst=1, xr=[base-30.d-6,base+30.d-6]
;     oplot, hrgrid, hrABCD[1,2,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,2,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,2,*], color=fsc_color('green')
;   plot, hrgrid, hrABCD[0,3,*], xst=1, xr=[base-35.d-6,base+35.d-6]
;     oplot, hrgrid, hrABCD[1,3,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,3,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,3,*], color=fsc_color('green')
;   plot, hrgrid, hrABCD[0,4,*], xst=1, xr=[base-35.d-6,base+35.d-6]
;     oplot, hrgrid, hrABCD[1,4,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,4,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,4,*], color=fsc_color('green')
;   plot, hrgrid, hrABCD[0,5,*], xst=1, xr=[base-40.d-6,base+40.d-6]
;     oplot, hrgrid, hrABCD[1,5,*], color=fsc_color('red')
;     oplot, hrgrid, hrABCD[2,5,*], color=fsc_color('yellow')
;     oplot, hrgrid, hrABCD[3,5,*], color=fsc_color('green')
;   !p.multi=[0,1,0]
...
stepsize = abs(median(ts_diff(hrgrid,1)))	;stepsize same for all pixels
lag = [dindgen(ndata-1.d0)-(ndata-1.d0), dindgen(ndata)]
nlag = n_elements(lag)
cc = dblarr(3,6,n_elements(lag))	;only '3' bacause CC between AB,AC,AD
ind_maxposcc = intarr(3,6,1)
dabcd = dblarr(3,6,1) & dabcd_rad = dabcd
angrad = dblarr(3,6,1) & angdeg = angrad
phserr = dblarr(4,6,1)

; define CC zero point to avoid CCmax confusion
for j=0,5 do begin

  dum = max(c_correlate(hrABCD[0,j,*], hrABCD[0,j,*], lag, /double),tmp)
  cc0lagidx = (j eq 0)?tmp:[cc0lagidx,tmp]
...
for i=0,2 do begin
  ...
  for j=0,5 do begin
    ...
    cc[i,j,*] = c_correlate(hrABCD[0,j,*], hrABCD[i+1,j,*], lag, /double)
;     dum = max(cc[i,j,*], tmp)
;     ind_maxposcc[i,j,*] = tmp
;     dabcd[i,j,*] = abs(lag[ind_maxposcc[i,j,*]])*stepsize
      ; define search area of one wavelength LEFT of cc0lag reference
;----------
    sidx = cc0lagidx[j]-indgen(ceil(lambda[j]/stepsize))
    dum = max(cc[i,j,sidx],tmp)
    tmp = (lag[cc0lagidx[j]] - lag[sidx[tmp]])
    if tmp lt 0 then stop,' Negative lag difference, stopping here ...'
    dabcd[i,j,*]     = tmp*stepsize
;----------
    dabcd_rad[i,j,*] = (dabcd[i,j,*]*2.d0*!DPI)/wlen[i,j]

    if (i eq 0) then angrad[i,j,*] = dabcd_rad[i,j,0]-(!DPI/2.d0)
    if (i eq 1) then angrad[i,j,*] = dabcd_rad[i,j,0]-!DPI
    if (i eq 2) then angrad[i,j,*] = dabcd_rad[i,j,0]-(3.d0*!DPI/2.d0)

  endfor

;   if (i eq 0) then begin
;     phserr[i,*,*] = sin(angrad[0,*,*])
;     angdeg[i,*,*] = 90.d0+angrad[i,*,*]/!dtor
;   endif
;   if (i eq 1) then begin
;     phserr[i,*,*] = 1.d0+cos(angrad[1,*,*])
;     angdeg[i,*,*] = 180.d0+angrad[i,*,*]/!dtor
;   endif
;   if (i eq 2) then begin
;     phserr[i,*,*] = cos(angrad[0,*,*])+cos(angrad[2,*,*])
;     angdeg[i,*,*] = 270.d0+angrad[i,*,*]/!dtor
;   endif


  proceeding_text,loop=(3), i=i, prompt='> Compute angular phase shifts   '+string(i+1,form='(I4)')

endfor

phserr[0,*,*] =   sin(angrad[1,*,*])
angdeg[0,*,*] = 90.d0+angrad[0,*,*]/!dtor

phserr[1,*,*] = 1.d0+cos(angrad[1,*,*])
angdeg[1,*,*] =   180.d0+angrad[1,*,*]/!dtor

phserr[2,*,*] = cos(angrad[0,*,*])+cos(angrad[2,*,*])
angdeg[2,*,*] = 270.d0+angrad[2,*,*]/!dtor

phserr[3,*,*] = -sin(angrad[0,*,*])-sin(angrad[2,*,*])
...
;   for j=0,5 do begin
; 
;     if (i eq 0) then if (angdeg[i,j] gt 180.d0) then angdeg[i,j] = 360.d0-angdeg[i,j]
;     if (i eq 1) then if (angdeg[i,j] gt 270.d0) then angdeg[i,j] = 360.d0-angdeg[i,j]
;     if (i eq 2) then if (angdeg[i,j] lt 180.d0) then angdeg[i,j] = 360.d0-angdeg[i,j]
; 
;   endfor
; 
; endfor
