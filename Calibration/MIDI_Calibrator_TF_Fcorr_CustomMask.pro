...
pro MIDI_Calibrator_TF_Fcorr_CustomMask
;FOR CALIBRATORS
;**********************************************************************************************
...
if (redmethod eq '1') then $
  resultsdir = '/media/disk_MIDIFSU/MIDI_FSUA/MIDI_FSUA_'+commnum+'/MIDIreduced_EWS_CustomMask/'
if (redmethod eq '2') then $
  resultsdir = '/media/disk_MIDIFSU/MIDI_FSUA/MIDI_FSUA_'+commnum+'/MIDIreduced_CohInt_CustomMask/'
if (redmethod eq '3') then $
  resultsdir = '/media/disk_MIDIFSU/MIDI_FSUA/MIDI_FSUA_'+commnum+'/MIDIreduced_Koresko_CustomMask/'
print, ''
...
openw, lun, pcrdir+'PhotonCountRate.txt', width=1400, /get_lun;, /append
...
;extract data from database
  ...
  diam = diamData.diameter	;arcseconds
  diamerr = diamData.diameter_err

  flux12 = photData.flux	;Jy, IRAS flux
  flux12err = photData.flux_err
...
;photon count rate, ADU/s/Jy
  ...
  openw, lun, workingdir+id[i]+'_'+time[i]+'.pcr', width=1400, /get_lun
    ...
    for j=0,n_elements(wl)-1 do begin
      printf, lun, wlout[j], pcr[j], format="(f10.5,f20.10)"
    endfor
...
wlall = dblarr(n,171) & pcrall = wlall
;window, 0, xs=1000, ys=800
for i=0,n-1 do begin
  readcol, resultsdir+id[i]+'_'+time[i]+'.pcr', wltmp, pcrtmp, format='d,d', /silent
  wlall[i,*] = wltmp
  pcrall[i,*] = pcrtmp
;  if (i eq 0) then $
...
;if (idxnight[0] ne -1) then begin
  ...
  for i=0,nnights-1 do begin
    ...
    openw, lun, pcrdir+night[2]+night[1]+night[0]+'_meanTF_Fcorr_Calibrator.txt', width=1400, /get_lun
    ...
    set_plot, 'ps'
    device, isolatin=1
    device, filename=pcrdir+night[2]+night[1]+night[0]+'_TF_Fcorr_Calibrator.ps', /color,XSIZE=23, YSIZE=17, XOffset=xoffset, YOffset=yoffset
...
;       yr[0] = 0.
    ...
    spawn, 'gv '+pcrdir+night[2]+night[1]+night[0]+'_TF_Fcorr_Calibrator.ps'
