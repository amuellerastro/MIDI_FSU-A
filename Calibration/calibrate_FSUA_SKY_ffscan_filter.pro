...
pro calibrate_FSUA_SKY_ffscan_filter, dodark=dodark, nodark=nodark
...
for i=0,nscans-1 do begin
  ...
  for j=0,5 do begin
    ...
    dum = max(c_correlate(hrABCD[i,0,j,*], hrABCD[i,0,j,*], lag, /double),tmp)
...
;       dabcd[i,j,k,*] = abs(lag[ind_maxposcc[i,j,k,*]])*stepsize
  ...
  ;----------
      sidx = cc0lagidx[k]-indgen(ceil(lambda[k]/stepsize))
      dum = max(cc[i,j,k,sidx],tmp)
      tmp = (lag[cc0lagidx[k]] - lag[sidx[tmp]])
      if tmp lt 0 then stop,' Negative lag difference, stopping here ...'
      dabcd[i,j,k,*]     = tmp*stepsize
  ;----------
...
;     endif
  ...
  phserrall[i,0,*,*] =   sin(angrad[i,1,*,*])
  angdegall[i,0,*,*] = 90.d0+angrad[i,0,*,*]/!dtor

  phserrall[i,1,*,*] = 1.d0+cos(angrad[i,1,*,*])
  angdegall[i,1,*,*] =   180.d0+angrad[i,1,*,*]/!dtor

  phserrall[i,2,*,*] = cos(angrad[i,0,*,*])+cos(angrad[i,2,*,*])
  angdegall[i,2,*,*] = 270.d0+angrad[i,2,*,*]/!dtor
