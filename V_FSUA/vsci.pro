...
;get alpha2 and delta2 
  ...
  vis2_px3[i] = (get_visibility_binary(flr, u[i], v[i], wl[3], alpha1, delta1, alpha2, delta2))^2.	;for quick check compute vis^2 using middle of K-band, assuming 3rd pixel of FSUA

  for j=0,5 do vis2_all[i,j] = (get_visibility_binary(flr, u[i], v[i], wl[j], alpha1, delta1, alpha2, delta2))^2.
